# Read significant trees from CAFE
# Connor Murray 1.23.2024
...
tre <- read.nexus(file="run1_filt_hogs/Significant_trees.edit.tre")
...
extract.expanding.genes <- function(spp.i) {
  ...
  write.table(ppp, file = paste("/scratch/csm6hg/data/", 
                                spp.out, ".expanding.feb16.genes.txt", sep=""), 
...
extract.contract.genes <- function(spp.i) {
  ...
  write.table(ppp, file = paste("/scratch/csm6hg/data/", 
                                spp.out, ".contract.feb16.genes.txt", sep=""), 
              quote = F, row.names = F, col.names = F)
...
GOtables_filenames <- list.files(pattern = "GOterm_mapping.tsv",
                                 path = "/scratch/csm6hg/genomes/proteins_species/hmmer_sep8",  
...
GeneSetCollection_constructor <- function(genome_GO_table = GOtable){
  ...
  #saveRDS(gmap, file = paste("/scratch/csm6hg/genomes/proteins_species/hmmer_sep8/gmap.", go_species, ".rds", sep=""))
...
go.test.clusterpro <- function(full_set, gene_lists_tbl_test, namei) {
  ...
  spp.gmap = readRDS(list.files(path = "../genomes/proteins_species/hmmer_sep8", 
  ...
  go.test2 <- data.table(go.test@result) %>% filter(p.adjust<0.01)
  ...
  ggsave(plot.go, filename = paste("../figs/", ".",namei,".", 
                                   go_species,".goterm.cafe.sep9.pdf", sep=""), width=20, height=12)
  
  # Write list for REVIGO 
  fwrite(x = pp %>% 
           select(ID, p.adjust, odd), 
         file = paste("../data/",".",namei,".", go_species,".goterm.cafe.sep9.revigo.txt", sep=""), 
...
revigo_list_contract <- list.files("/scratch/csm6hg/data", 
                                   pattern = "contract.feb13.revigo", full.names = T)
revigo_list_expand <- list.files("/scratch/csm6hg/data", 
                                 pattern = "expand.feb13.revigo", full.names = T)
...
# Save output
saveRDS(go.reduce.out.con, file = "/scratch/csm6hg/data/reduced.terms.contracting.feb26.rds")
saveRDS(go.reduce.out.con, file = "/scratch/csm6hg/data/reduced.terms.expanding.feb26.rds")
